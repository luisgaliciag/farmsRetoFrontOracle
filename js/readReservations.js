...
function listar() {
    $.ajax({
        url: "http://143.47.234.84:8080/api/Reservation/all",
        type: "GET",
...
function loadReservas(items){
    ...
    for (var i=0; i < items.length; i++) {
        dataHtml += `<tr>
                ...
                <td>${items[i].score}</td>
                <td><button class="btn btn-outline-success btn-sm" onclick="editarRegistro(${items[i].idReservation})">Editar</button></td>
                <td><button class="btn btn-outline-danger btn-sm" onclick="borrarRegistro(${items[i].idReservation})">Borrar</button></td>
            </tr>`;
    }
